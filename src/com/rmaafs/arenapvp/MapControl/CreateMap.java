...
public class CreateMap {
    ...
    public CreateMap(Player player, Kit kit) {
        this.player = player;
        this.kit = kit;
        setConfig();
    }
    ...
    public void paso() {
        Extra.playSound(player, ORB_PICKUP);
        switch (action) {
            case CORNER1:
                player.sendMessage(corner.replaceAll("<number>", "1"));
                cleanPlayer(player);
                player.getInventory().addItem(new ItemStack(Material.getMaterial(35), 1, (byte) 14));
                break;
            case CORNER2:
                player.sendMessage(corner.replaceAll("<number>", "2"));
                break;
            case SPAWN1:
                player.sendMessage(spawn.replaceAll("<number>", "1"));
                cleanPlayer(player);
                break;
            case SPAWN2:
                player.sendMessage(spawn.replaceAll("<number>", "2"));
                break;
            case NAME:
    ...
    public void createMap(String s) {
        ...
        createFiles(s);
        player.sendMessage(created.replaceAll("<kit>", kit.getKitName()).replaceAll("<map>", m.getName()));
        creandoMapa.remove(player);
        cleanPlayer(player);
        hotbars.setMain(player);
        extraLang.teleportSpawn(player);
        Extra.playSound(player, LEVEL_UP);
    }
